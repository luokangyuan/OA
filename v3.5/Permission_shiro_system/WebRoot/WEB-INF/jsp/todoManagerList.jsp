...
<html>
  ...
  <body>
    ...
    <div data-options="region:'center',border:false">
    	<!-- 表格工具开始 -->
        <div id="todomanagtoolbar">
         <div class="wu-toolbar-button">
                <a href="#" class="easyui-linkbutton" iconCls="icon-add" onclick="openAddTodoManager()" plain="true">添加事项</a>
                <a href="#" class="easyui-linkbutton" iconCls="icon-edit" onclick="openEdittodo()" plain="true">修改事项</a>
                <a href="#" class="easyui-linkbutton" iconCls="icon-remove" onclick="removetodo()" plain="true">删除事项</a>
            ...
            <div class="wu-toolbar-search">
                ...
                <select id="todoimpor" name="todoimpor" class="easyui-combobox" editable=false panelHeight="auto" style="width:100px">
                 ...
                 <label>下属员工：</label> 
                <select id="lowstaffs" name="staId" class="easyui-combobox" editable=false panelHeight="auto" style="width:100px">
        ...
        <!-- 表格工具结束 -->
        <table id="todomdata" toolbar="#todomanagtoolbar"></table>
    </div>
</div>
<!-- 新增弹出窗口开始 -->
<div id="sysdialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-add'" style="width:400px;  padding:10px;">
	<form id="sysform" enctype="multipart/form-data" method="post">
        ...
        <div style="padding:20px 0px;margin-left:70px">
            ...
            <div class="my_li">
                <div class="my_li_float">重要级别：</div>
                <div class="my_li_float"> 
             ...
             <div class="my_li">
                <div class="my_li_float">选择员工：</div>
                <div class="my_li_float"> 
                 <select  id="lowstaff" name="lowstaff"   editable=false class="easyui-combobox easyui-validatebox"  panelHeight="auto" style="width:200px">
...
<script type="text/javascript">
 ...
 $('#lowstaffs').combobox({
	...
	*/
	function removetodo(){
		var item = $('#todomdata').datagrid('getSelections');
		if(item.length <= 0){
			$.messager.show({
				title:'提示信息！',
				msg:'至少选择一行进行删除'
				...
				$.messager.confirm('提示信息','确认删除吗？', function(r){
					if(r){
						//删除用户
							...
							$.post('deletetodo.action',{idlist:ids},function(result){
								//删除成功
								//刷新数据表格
								$('#todomdata').datagrid('reload');
								$('#todomdata').datagrid('unselectAll');
								$.messager.show({
									title:'提示信息',
									msg:'删除成功！'
								});
		...
		if(item.length != 1){
			...
			$('#sysdialog').dialog('open');
				$('#sysform').form('load',{
					...
					todoTitle:item[0].todoTitle,
					todoContent:item[0].todoContent,
					todoimpor:item[0].todoimpor,
...
//加载下属员工信息
 $('#lowstaff').combobox({
	...
	*/
	function inserttodo(){
		if($('#sysform').form('validate')){
			$('#sysform').form('submit', {
			url:'insertTodoWorkBysystem.action',
			success:function(data){
				if(data){
					$.messager.show({
							title:'提示信息',
	...
	$('#querytodo').click(function(){
		$('#todomdata').datagrid('load',serializeForm($('#selectNoticeByCondition')));
	});
	//清空查询条件
	...
	*/
	$('#todomdata').datagrid({
		...
		striped:true,    //隔行变色
		url:'loadTodoManagerList.action',
		loadMsg:'数据正在加载，请等待',
		rownumbers:true,
		remoteSort:false,
		multiSort:false,
		...
		columns:[[
			...
			{ field:'todoId',title:'id',width:70,hidden:true},
			{ field:'staname',title:'执行人',width:70,sortable:true},
